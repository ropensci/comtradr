...
test_that("lookup return values are correct, and fail when expected", {
  ...
  ex_1 <- ct_commodity_lookup(values = "halibut",
                              ...
                              return_char = TRUE,
                              verbose = TRUE)
  ex_2 <- ct_commodity_lookup(values = 1602,
                              lookuptable = df,
                              return_code = TRUE,
                              return_char = FALSE,
                              verbose = TRUE)
  ex_3 <- ct_commodity_lookup(values = df$commodity[c(10, 20, 40)],
                              lookuptable = df,
                              return_code = TRUE,
                              return_char = TRUE,
                              verbose = TRUE)
  ex_4 <- ct_commodity_lookup(values = c("tomato", "trout"),
                              lookuptable = df,
                              return_code = FALSE,
                              return_char = FALSE,
                              verbose = TRUE)
  ...
  expect_error(ct_commodity_lookup(value = list(), lookuptable = df))

  # Throw error with invalid input for param "lookuptable".
  expect_error(ct_commodity_lookup(values = "halibut",
