...
* For the same group of three (`reporters`, `partners`, date range), if the input is not "All", then the maximum number of input values for each is five. For date range the `start_date` and `end_date` must not span more than twelve months or twelve years. There is no more parameter to specify `All` years.
