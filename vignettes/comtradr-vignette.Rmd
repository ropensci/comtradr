...
```
If we want to search for shipment data on all of the commodity descriptions listed, then we can simply adjust the params for `commodity_lookup` so that it will output only the codes, which can then be passed along to the API call.
```{r, eval = FALSE}
